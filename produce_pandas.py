...
produce_df = produce.T

# 1. Produce that had the highest and lowest sales in total sales
print("#1) -----------------------------")
print("Highest sales produce:", produce_df["Total Sale"].idxmax())
print("Lowest sales produce:", produce_df["Total Sale"].idxmin())
print()

# 2. Using 'loc', display the quantity and total sales for 'Orange' and 'Beets' (together)
print("#2) -----------------------------")
print(produce_df.loc[["Orange", "Beets"], ["Quantity Sold", "Total Sale"]])
print()

# 3.  Using 'at', display the total sales for 'Cucumber'
print("#3) -----------------------------")
print("Total Sales for Cucumber is:", produce_df.at["Cucumber", "Total Sale"])
print()

# 4. Create a new dataframe for only those produce that have sold between 11,500 to 12,000 (quantity)
print("#4) -----------------------------")
new_df = produce_df[
    (produce_df["Quantity Sold"] >= 11500) & (produce_df["Quantity Sold"] <= 12000)
]
print(new_df)
print()

# 5. Create a copy of the data frame by transposing it so that the columns are produce names and rows are cost, quantity and total sales
print("#5) -----------------------------")
produce_col_df = produce_df.T
print(produce_col_df)
